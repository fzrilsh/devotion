...
### Diperbaiki
- Perbaikan alur request kuota menutup beberapa celah yang membuat data dan tindakan tidak konsisten. Periode minggu berjalan yang hilang kini dibuat ulang saat kalender diperbarui tanpa menimpa nilai periode yang sudah ada; deadline request di masa lampau ditolak dengan validasi field; tautan notifikasi counter-offer untuk subkontraktor mengarah ke detail incoming, sedangkan tautan untuk buyer tetap ke request terkirim. Ditambahkan `GET /api/candidates/{candidateId}` yang memeriksa pemilik listing dan mengembalikan detail request, kapasitas, serta seluruh riwayat penawaran sehingga halaman incoming dapat dibuka langsung atau setelah refresh. Uji recovery juga memastikan akun belum terverifikasi dan permintaan kode kedua tetap menerbitkan serta mengirim kode baru (FR-003, FR-017, FR-029, FR-030, FR-031, FR-033, FR-090).
