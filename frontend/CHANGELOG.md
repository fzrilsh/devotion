...
* Menyelaraskan semua validasi form frontend dengan kontrak dan backend: `machines` pada listing kini memakai `.min(1, "Pilih minimal satu mesin.")`, `readiness_lead_days` mengikuti maksimum 365 hari, `material` mengikuti `minLength: 1` dan `maxLength: 200`, `proposed_name` mengikuti `minLength: 2`, alasan pembatalan pesanan mengikuti `minLength: 5` dan `maxLength: 500`, alasan penolakan/penyembunyian mengikuti `minLength: 5` serta `maxLength: 1000`, dan semua catatan counter-offer serta laporan sengketa/review menaruh `maxLength` yang sesuai dengan OpenAPI. Pesan galat di UI kini menyebut batas yang benar dalam bahasa Indonesia, dan atribut HTML `minLength`/`maxLength` serta `min`/`max` diinput disamakan agar bantuan browser dan pembaca layar konsisten dengan validasi runtime.

* Memperbaiki root cause pada keputusan sengketa admin di `src/pages/Admin/Disputes.tsx`: pemilih pihak yang bertanggung jawab hanya dirender ketika data pesanan sudah siap, sementara saat query masih memuat atau gagal halaman menampilkan status yang jelas, tombol "Putuskan Sengketa" nonaktif, dan pesan galat tidak lagi menyuruh memilih pihak yang tidak ada di layar. Ini menjaga UI selaras dengan aturan backend yang mewajibkan `liable_profile_id` untuk cabang `cancelled` dan mencegah submit yang tidak mungkin dilakukan ketika data order belum tersedia.

* Menutup temuan kecil yang tersisa saat audit frontend terhadap `openapi.yaml`: peta profil tidak lagi dirender saat koordinat null dengan pesan yang jelas dalam bahasa Indonesia, fallback `readiness_deadline ?? deadline` dihapus dari halaman admin, tautan notifikasi sekarang mengikuti target yang dikirim server tanpa penyaringan peran yang tidak konsisten, kalender tidak lagi menghitung Senin awal minggu sendiri lewat `currentMondayJakarta`, 400 pada konfirmasi pemulihan kata sandi mendapat pesan yang jelas, nomor telepon di registrasi dikirim dalam format `62...` yang sesuai dengan kontrak backend, dan path profil publik memakai `encodeURIComponent` agar identifier tidak melanggar URL.
